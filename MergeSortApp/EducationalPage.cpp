...
unsigned long long if_countEdu; // iloœæ zliczonych porównañ
unsigned long long arr_accessEdu; // iloœæ zliczonych dostêpów do tablic
unsigned long long arr_changedEdu; // iloœæ zliczonych zmian w tablicy
        ...
        : TForm(Owner)
{
    ...
    if_countEdu = 0; // Nadanie pocz¹tkowej wartoœci licznika if'ów
    arr_accessEdu = 0; // Nadanie pocz¹tkowej wartoœci licznika dostêpów do tablicy
	arr_changedEdu = 0; // Nadanie pocz¹tkowej wartoœci licznika zmian w tablicy
...
void merge_App(T *arr, int l, int  s, int p){
   ...
   while(i <= s && j <= p)
   {
      ++if_countEdu; // Element aplikacji
      arr_accessEdu += 2; // Element aplikacji
      if(arr[i] <= arr[j])
	  {
         pom[k++] = arr[i++];
		arr_accessEdu += 2; // Element aplikacji
		arr_changedEdu += 1; // Element aplikacji
	  }
	  else
	  {
         pom[k++] = arr[j++];
...
{
    int tab_length = 0.001 * length;
    if(length < 5000 && length >= 500)
        tab_length = 0.01 * length;
    else if(length < 500 && length >= 50)
...
void TfEducationalPage::checkAmount()
{
    if( !isAmountGood() )
    {
        ShowMessage("Size of the table: Invalid integer value entered, \ntry again between "
...
void TfEducationalPage::checkRepeat()
{
    if( !isRepeatGood() )
    {
        ShowMessage("Repeat: Invalid integer value entered, try again.");
        eRepeat->Text = 1;
        eRepeat->SetFocus();
    }
}
...
void __fastcall TfEducationalPage::About1Click(TObject *Sender)
{
